...
//  Drumed
//
//  Created by Andrew Donnelly on 25/07/2021.
...
class SliderHandle: ObservableObject {
    ...
    //@Binding var currentProgress: CGPoint
    ...
    init(sliderWidth: CGFloat, sliderHeight: CGFloat, sliderValueStart: Double, sliderValueEnd: Double, startPercentage: SliderValue) {
        ...
        let progressLocation = CGPoint(x: (CGFloat(startPercentage.wrappedValue)/1.0)*sliderWidth, y: sliderHeight/2)
...
class CustomSlider: ObservableObject {
    ...
    @Published var progressHandle: SliderHandle
    ...
    @SliderValue var progressHandleStartPercentage = 0.0

    final var anyCancellableHigh: AnyCancellable?
    final var anyCancellableLow: AnyCancellable?
    final var anyCancellableProgress: AnyCancellable?
    ...
    init(start: Double, end: Double) {
        ...
        progressHandle = SliderHandle(sliderWidth: width,
                                  sliderHeight: lineWidth,
                                  sliderValueStart: valueStart,
                                  sliderValueEnd: valueEnd,
                                  startPercentage: _progressHandleStartPercentage
                                )
        ...
        anyCancellableProgress = progressHandle.objectWillChange.sink { _ in
            self.objectWillChange.send()
        }
    }
    ...
    var currentProgressValue: String {
        return String(format: "%.2f", progressHandle.currentValue)
    }
}
